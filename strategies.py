...
def Breakout(df):
    ...
    """
    # Donchian Channel
    df['20d_high'] = np.round(pd.Series.rolling(df['Adj Close'], window=20).max(), 2)
    df['10d_low'] = np.round(pd.Series.rolling(df['Adj Close'], window=10).min(), 2)
...
def BBand(df):
    ...
    df['5ma'] = pd.Series.rolling(df['Adj Close'], window=5).mean()
    # bbands策略,N=20
    df['20ma'] = pd.Series.rolling(df['Adj Close'], window=20).mean()
    df['SD'] = pd.Series.rolling(df['Adj Close'], window=20).std()
    ...
    for t in range(2, df['signals'].size):
        if  (df['5ma'][t] > df['lowbbands'][t-1]):
            if not has_position:
                df.loc[df.index[t], 'signals'] = 1
                has_position = True
        elif  (df['Adj Close'][t] < df['midbbands'][t-1]):
            if has_position:
                df.loc[df.index[t], 'signals'] = -1
                has_position = False
...
def WMSR(df):
    ...
    df['WMSR'] = 100*((df['high9'] - df['Adj Close']) / (df['high9'] - df['low9']) )
...
def BIAS(df):
    ...
    """
    has_position = False
    df['6d'] = pd.Series.rolling(df['Adj Close'], window=6).mean()
    df['BIAS'] = (df['Adj Close'] - df['6d'] )/df['6d']
    df['signals'] = 0
    for t in range(2, df['signals'].size):
        if df['BIAS'][t] < -0.03:
            if not has_position:
...
def MACD(df):
    ...
    """
    df['EMAfast'] = pd.Series.ewm(df['Adj Close'], span = 12).mean()
    df['EMAslow'] = pd.Series.ewm(df['Adj Close'], span = 26).mean()
