...
def draw(df, title="", colorup='g', colordown='r', signal=True, figsize=(10,8), fontname=None, save=False, **kwargs):
    # 畫出價量曲線
    ...
    font = None
    if fontname:
        font_path = os.path.join(os.path.dirname(__file__), fontname)
        if os.path.exists(font_path):
            from matplotlib.font_manager import FontProperties
            font = FontProperties(fname=font_path, size=24)
    ...
    last_price = "Date:{}, Open:{:.2f}, High:{:.2f}, Low:{:.2f}, Close:{:.2f}, Volume:{}".format(df.Date[-1], df.Open[-1], df.High[-1], df.Low[-1], df.Close[-1], df.Volume[-1])
    ...
    if 'price' in kwargs.keys():
        for i in kwargs['price']:
            ax0.plot(df[i].values, lw=2, label=i)

    # 畫成交量相關曲線
    if 'volume' in kwargs.keys():
        for i in kwargs['volume']:
            ax0.plot(df[i].values, lw=1, label=i)

    # 標出進出場訊號
    if signal:
        if 'signals' in df.columns:
            # Plot the "buy" trades
            ax0.plot(df.loc[df.signals == 1].index,df['Close'][df.signals == 1],'^', markersize=10, color='r')

            # Plot the "sell" trades
            ax0.plot(df.loc[df.signals == -1].index, df['Close'][df.signals == -1], 'v', markersize=10, color='k')
